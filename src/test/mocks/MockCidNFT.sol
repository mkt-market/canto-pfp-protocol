...
contract MockCidNFT {
    ...
    function getPrimaryCIDNFT(string memory subprotocolName, uint256 pfpId) public view returns (uint256) {
        if (expectedPfpId != 0) assert(expectedPfpId == pfpId);
        assert(keccak256(abi.encodePacked(subprotocolName)) == keccak256(abi.encodePacked("SubprotocolName")));
